...
const NETWORK_SNIFFER_URL = process.env.NETWORK_SNIFFER_URL || 'http://127.0.0.1:9432/events';
const ENABLED = process.env.NETWORK_SNIFFER_ENABLED !== 'false'; // Enabled by default
...
export function setupNetworkSniffer(
  ...
  if (!ENABLED) {
    console.log('[Network Sniffer] Disabled (set NETWORK_SNIFFER_ENABLED=true to enable)');
    return;
  }
